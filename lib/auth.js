...
export function AuthProvider({ children }) {
  ...
  const followPolitician = async (id) => {
  ...
  const unfollowPolitician = async (id) => {
    if (!user) return false;
    const updated = (profile?.followed_politicians || []).filter(s => s !== id);
    const { error } = await supabase.from("profiles").update({ followed_politicians: updated }).eq("id", user.id);
    if (!error) setProfile(p => ({ ...p, followed_politicians: updated }));
    return !error;
  };
  ...
  const isFollowingPolitician = (id) => profile?.followed_politicians?.includes(id) || false;
