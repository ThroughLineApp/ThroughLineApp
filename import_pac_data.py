# import_pac_data.py
# Downloads FEC bulk PAC-to-candidate data and loads into Supabase
# Run: python import_pac_data.py
...
# [15] OTHER_ID
# [16] CAND_ID         — FEC candidate ID, e.g. "S4WV00159"
# [17] TRAN_ID
# [18] FILE_NUM
# [19] MEMO_CD
# [20] MEMO_TEXT
...
def load_politicians():
    print("Loading politicians from Supabase...")
    status, data = supabase_request(
        "GET",
        "politicians?select=id,name,bioguide_id,state,chamber,slug&is_current=eq.true&limit=600"
    )
...
def load_fec_candidate_ids():
    ...
    so we capture senators on all 6-year rotation schedules.
    Returns candidates_by_key (last_state_office → cand_id) and fec_ids dict.
    """
    candidates_by_key = {}
    fec_ids = {}
        ...
        with zipfile.ZipFile(io.BytesIO(zip_bytes)) as z:
            ...
            with z.open(filename) as f:
                ...
                for row in reader:
                    ...
                    key = f"{last_raw}_{state}_{office}"
                    if key not in candidates_by_key:
                        candidates_by_key[key] = cand_id
                        count += 1
        print(f"  Added {count} new entries from {cycle_label}")

    print(f"  Total FEC candidates indexed: {len(fec_ids)}")
...
def match_politicians_to_fec(politicians, fec_by_key, fec_by_id):
    ...
    for pol in politicians:
        office = "S" if pol["chamber"] == "Senate" else "H"
        ...
        # Try compound: last two words joined (e.g. "Cortez Masto")
        ...
        if fec_cand_id:
            # Prefer exact office match to avoid key collisions
            # e.g. "Ted Cruz" (Senate) and "Monica De La Cruz" (mislabeled as Senate) both → cruz_TX_S
            existing = matched.get(fec_cand_id)
            if not existing:
                matched[fec_cand_id] = pol
            else:
                # Keep whichever politician's name/chamber better fits the FEC office
                # FEC IDs starting with S = Senate, H = House
                fec_office = "Senate" if fec_cand_id.startswith("S") else "House"
                if pol["chamber"] == fec_office and existing["chamber"] != fec_office:
                    matched[fec_cand_id] = pol
        else:
            unmatched.append(f"{pol['name']} (tried: {last_candidates[0]}, state: {state}, office: {office})")

    print(f"  Matched {len(matched)} of {len(politicians)} politicians to FEC IDs")
    if unmatched[:8]:
...
def load_pac_contributions(matched_politicians):
    ...
    print(f"  Found {len(contributions)} PAC contributions to our politicians")
...
def save_to_supabase(contributions):
    ...
    print(f"  Saving {len(rows)} aggregated PAC donor records to Supabase...")
    BATCH = 100
    saved = 0
    errors = 0
    for i in range(0, len(rows), BATCH):
        batch = rows[i:i+BATCH]
        status, result = supabase_request("POST", "pac_donors", batch)
        if status in (200, 201):
            saved += len(batch)
        else:
            errors += len(batch)
            print(f"  Insert error (batch {i//BATCH}): {result[:300]}")
    print(f"  Saved {saved} records, {errors} errors")

def main():
    print("=" * 50)
    print("  THROUGHLINE PAC DATA IMPORTER")
    print("=" * 50)
