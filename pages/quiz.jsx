...
const C = {
  ...
  gold:"#c9a84c", goldDim:"#8a6e30",
  goldBorder:"rgba(201,168,76,0.35)", goldBorderDim:"rgba(201,168,76,0.12)",
  parchment:"#e8dfc8", parchmentDim:"#a89d88",
  green:"#4ca87c", red:"#c94c4c", blue:"#4c78c9", purple:"#8e4cc9",
};
...
export default function QuizPage() {
  ...
  const commitAndAdvance = (newAnswers, newWritten) => {
    ...
    if (qIndex + 1 >= SHUFFLED.length) {
      setPhase("processing");
      runProcessing(newAnswers, newWritten);
    } else {
      setQIndex(i => i + 1);
    }
  };
  ...
  const handleNext = () => {
    ...
    const writtenText = writeOwn
      ? ownText.trim()
      : (deeperOpen && written[q.dimension]?.trim() ? written[q.dimension].trim() : null);
    const newAnswers = { ...answers, [q.dimension]: score };
    const newWritten = writtenText ? { ...written, [q.dimension]: writtenText } : written;
    transition("forward", () => commitAndAdvance(newAnswers, newWritten));
  };
  ...
  const handleSkip = () => {
    ...
    transition("forward", () => commitAndAdvance(newAnswers, written));
  };

  // ── PROCESSING ── guaranteed to always reach setPhase("results")
  const runProcessing = async (rawAnswers, writtenInputs) => {
    try {
      setProcessingMsg("Reading your answers…");

      // Build numeric scores (exclude skipped)
      const numeric = {};
      Object.entries(rawAnswers).forEach(([k, v]) => {
        if (v !== "skipped") numeric[k] = v;
      });
      let refined = { ...numeric };

      // AI refinement for written responses
      const writtenEntries = Object.entries(writtenInputs).filter(([, v]) => v && v.trim());
        ...
        try {
          const res = await fetch("https://api.anthropic.com/v1/messages", {
            ...
            body: JSON.stringify({
              ...
              messages: [{
                role: "user",
                content: `Adjust political scores (0=most conservative, 100=most progressive) based on written quiz responses.
Current scores: ${Object.entries(numeric).map(([k,v]) => `${k}:${v}`).join(", ")}
Written responses: ${writtenEntries.map(([k,v]) => `${k}: "${v}"`).join(" | ")}
...
No markdown. No explanation. Just JSON.`
          ...
          refined = { ...numeric, ...aiParsed };
        } catch (aiErr) {
          console.log("AI refinement skipped:", aiErr.message);
        }
      }

      // Build full scores (fill missing with 50)
      setProcessingMsg("Building your political thumbprint…");
      await new Promise(r => setTimeout(r, 600));
      const fullScores = {};
      DIMS.forEach(d => { fullScores[d] = refined[d] ?? 50; });
      setScores(fullScores);

      // Politician matching (skips gracefully if no data)
      ...
      try {
        ...
        console.log("Matching skipped:", matchErr.message);
      }
      ...
      try {
        const row = {
          ...
          tier: "basic",
          score_economic:    fullScores.economic,
          score_healthcare:  fullScores.healthcare,
          score_climate:     fullScores.climate,
          score_criminal:    fullScores.criminal,
          score_immigration: fullScores.immigration,
          score_foreign:     fullScores.foreign,
          score_education:   fullScores.education,
          score_freedom:     fullScores.freedom,
          score_guns:        fullScores.guns,
          score_housing:     fullScores.housing,
          score_tech:        fullScores.tech,
          score_voting:      fullScores.voting,
          profile_summary: (() => {
            const avg = Object.values(fullScores).reduce((a, b) => a + b, 0) / 12;
            if (avg > 78) return "Progressive";
            if (avg > 62) return "Center-Left";
        ...
        const { data: saved, error: saveErr } = await supabase
          .from("quiz_results").insert(row).select().single();
        if (!saveErr && saved) {
          if (user?.id) {
            await supabase.from("profiles").update({ quiz_result_id: saved.id }).eq("id", user.id);
          } else {
            setShowSavePrompt(true);
          }
        }
      } catch (saveErr) {
        console.log("Save skipped:", saveErr.message);
      }

    } catch (outerErr) {
      console.log("Processing error:", outerErr.message);
    }
    ...
    <>
      ...
      <div style={{ minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 24px",textAlign:"center" }}>
        ...
        <div style={{ maxWidth:520,animation:"fadeSlideIn 0.6s ease forwards" }}>
          ...
          <p style={{ fontFamily:"'Figtree',sans-serif",fontSize:15,color:C.parchmentDim,lineHeight:1.8,marginBottom:12 }}>
  ...
  // ── RESULTS ───────────────────────────────────────────────────────────────
  if (phase === "results") {
    const safeScores = scores || Object.fromEntries(DIMS.map(d => [d, 50]));
    return (
      <>
        <Head><title>Your Political Thumbprint · Throughline</title></Head>
        <style>{GLOBAL_STYLES}</style>
        <div style={{ minHeight:"100vh",background:C.bg,color:C.parchment }}>
          <div style={{ maxWidth:580,margin:"0 auto",padding:"44px 24px 80px" }}>
            ...
            <div style={{ display:"flex",justifyContent:"center",marginBottom:12,animation:"fadeSlideIn 0.6s ease forwards 0.2s",opacity:0 }}>
              <ThumbprintSVG scores={safeScores} size={290} animate={true}/>
            </div>
                ...
                {matches.map(pol => {
                  ...
                  return (
                    <div key={pol.slug} onClick={() => router.push(`/politician/${pol.slug}`)}
                      style={{ display:"flex",alignItems:"center",gap:14,padding:"14px 16px",background:C.bgCard,border:`1px solid ${C.goldBorderDim}`,borderRadius:4,marginBottom:8,cursor:"pointer",transition:"border-color 0.15s" }}
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:10,animation:"fadeSlideIn 0.6s ease forwards 0.8s",opacity:0 }}>
              ...
              <button onClick={() => { setPhase("intro");setQIndex(0);setAnswers({});setWritten({});setSelectedIdx(null);setScores(null);setMatches([]);setShowSavePrompt(false); }}
            ...
            <div style={{ display:"flex",gap:10,alignItems:"center",marginTop:6 }}>
              {qIndex > 0 && (
                <button onClick={handleBack}
                  style={{ fontFamily:"'Figtree',sans-serif",fontWeight:600,fontSize:13,color:C.parchmentDim,background:"transparent",border:"1px solid rgba(255,255,255,0.1)",borderRadius:4,padding:"13px 18px",cursor:"pointer",flexShrink:0 }}
              ...
              {!skipWarning && skippedCount < MAX_SKIPS && (
                <button onClick={handleSkip}
                  style={{ fontFamily:"'Figtree',sans-serif",fontWeight:600,fontSize:12,color:C.parchmentDim,background:"transparent",border:"1px solid rgba(255,255,255,0.08)",borderRadius:4,padding:"13px 14px",cursor:"pointer",flexShrink:0 }}
